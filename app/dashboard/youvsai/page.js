...
async function getAIdata() {
  try {
    const res = await fetch(`${process.env.API}/api/ai`, {
      method: "GET",
      next: { revalidate: 21600 },
      headers: {
        "Content-Type": "application/json",
...
async function getData(x) {
  try {
    const res = await fetch(`${process.env.API}/api/fpl/${x}`, {
      method: "GET",
      next: { revalidate: 21600 },
      headers: {
        "Content-Type": "application/json",
...
export const metadata = {
  title: "FPL Mstr AI",
  description: "Your FPL team with AI.",
};
...
export default async function page() {
  ...
  const ai_data = await getAIdata();
  const allPoints = getAllPoints(data.my_team);
  const allAIPoints = getAllPoints(ai_data.ai);
        ...
        <div className="m-3 rounded shadow-sm overflow-auto bg-gradient-to-b from-[#02efff] to-white to-40% lg:w-2/4">
          ...
          <div className="m-0 min-w-0 pt-8">
            <div className="min-h-[524px] pt-1 bg-[length:625px_460px] md:bg-[length:938px_690px] bg-[url('/pitch.svg')] bg-no-repeat bg-top">
              ...
              <div className="flex max-w-md m-auto justify-around">
                {ai_data.ai
                  .filter((player) => player.element_type === 4)
                  .map((player) => (
                    <PlayerItem
